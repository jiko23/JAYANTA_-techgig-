...
def model_1(data,category) :
	...
	model = KMeans(n_clusters=num_clusters,verbose=1, n_init=20, max_iter=550,random_state=0).fit(x)
...
def final_model(data):
	...
	for i in output :
		model = Model(inputs=x_in,outputs=output1) ## DEFINING THE MODEL WITH INPUT AND OUTPUT
		...
		model.fit(x_train,y_train1,batch_size=120,epochs=550,shuffle=True,verbose=1) ## PASSING THE TRAINING FEATURES AND LABELS TO THE MODEL

		train_score = model.evaluate(x_train,y_train1,batch_size=120, verbose=1) ## TRAINING EVALUATION ALONG WITH LOSS AND ACCURACY SCORE
